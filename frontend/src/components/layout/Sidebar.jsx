...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <aside className={`w-xs h-screen bg-deeper border-r-1 border-white/5 flex flex-col p-5 fixed left-0 top-0 z-100 transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:sticky`}>
      <div className="flex items-center gap-4 mb-6 px-3">
        <div className="w-11 h-11 bg-primary/10 border border-glass-border rounded-md flex items-center justify-center shadow-glass-border">
          <Zap size={24} className="text-primary" fill="currentColor"/>
        </div>
        <div className="flex flex-col">
          <span className="text-2xl font-extrabold text-white">ALMA</span>
          <span className="text-xs font-semibold text-primary tracking-widest">RDC</span>
        </div>
        <button
      ...
      <nav className="sidebar-nav">
        <p className="text-md uppercase tracking-wider text-gray-500 opacity-50 mb-sm px-xs">Menu Principal</p>
        <div className="flex flex-col gap-1 px-xs">
          {menuItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-sm py-3 px-sm rounded-md text-white font-medium text-sm transition-all duration-200
                 ${isActive ? 'bg-primary/10 text-primary border-r-4 border-primary' : 'hover:bg-white/3 hover:text-white'}`
              }
            >
        ...
        <p className="text-md uppercase tracking-wider text-gray-500 opacity-50 mt-4 mb-sm px-xs">Administration</p>
        <div className="flex flex-col gap-1 px-xs">
          <NavLink to="/admin" className="flex items-center gap-sm py-3 px-sm rounded-md text-white font-medium text-sm hover:bg-white/3 hover:text-white">
            <ShieldCheck size={20} />
